  ...
  return (
    <button
      ...
      className={clsx(
        ...
        copied
          ? 'bg-brand-primary-400/10 ring-1 ring-brand-primary-400/20 ring-inset'
    ...
    >
      ...
      <span
        aria-hidden={!copied}
        className={clsx(
          'pointer-events-none absolute inset-0 flex items-center justify-center text-brand-primary-400 transition duration-300',
          !copied && 'translate-y-1.5 opacity-0',
        )}
      >
        Copied!
...
function CodePanel({
  ...
  // Helper to extract plain text from Shiki-processed HTML
  ...
  // Try to get single child, fall back to first child if multiple
  let child
  try {
    child = Children.only(children)
  } catch (error) {
    // Multiple children detected - use first one
    const childArray = Children.toArray(children)
    child = childArray[0]
    console.warn('[CodePanel] Multiple children detected, using first child')
  }

  // Extract props from child element
  if (isValidElement(child)) {
    ...
    // If code is still missing, extract from Shiki-processed children
    if (!code && typeof props.children === 'string') {
      code = extractPlainText(props.children)
    }
  }

  // Final fallback: extract from children directly if it's a string
  if (!code && typeof children === 'string') {
    code = extractPlainText(children)
  }

  if (!code) {
    console.error('[CodePanel] Could not extract code:', { child, children })
    throw new Error(
            ...
            <Tab
              className={clsx(
                ...
                childIndex === selectedIndex
                  ? 'border-brand-primary-500 text-brand-primary-400'
